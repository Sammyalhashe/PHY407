...
d.loc_o_new      = numarray.zeros((res,res,res),'f')
d.loc_o_pure     = numarray.zeros((res,res,res),'f')
d.loc_o_further  = numarray.zeros((res,res,res),'f')
d.loc_dens_tmp   = numarray.zeros((res,res,res),'f')
...
def timestep(i_step):
    ...
    if block_count > d.termalization:
        d.loc_o_new   = d.w_block.walkers2grid3D(d.loc_o_new)
        d.loc_o_pure += d.loc_o_new
        d.loc_o_new  *= 0.
        d.unit_pure  += len(d.w_block)
    ...
    if block_count > d.termalization:
        d.loc_o_new     = d.w_block.walkers2grid3D(d.loc_o_new)
        d.loc_dens_tmp += d.loc_o_new
        d.loc_o_new    *= 0.
        d.unit_mix     += len(d.w_block)
...
while d.tau_list: #loop over taus
	...
	#    d.pypar.barrier()
        ...
        if block_count > d.termalization:
            ...
            #                                   float(len(d.w_block)*d.steps))
            #for i in xrange(len(d.w_block)):
            #    d.w_block.setOPureFurther(i,d.w_block.getOPure(i))
            d.loc_dens_array += d.loc_dens_tmp/d.unit_mix
            d.loc_dens_pure += d.loc_o_further/d.unit_pure
            d.loc_o_further  = copy.deepcopy(d.loc_o_pure)
    ...
    tmp_dens_pure = d.pypar.gather(Numeric.array(d.loc_dens_pure),
                                   d.master_rank)
    tmp_dens_array = d.pypar.gather(Numeric.array(d.loc_dens_array),
                                      d.master_rank)
    if d.master:
        ...
        weight=1./float(d.blocks-d.termalization)
        os.system("mkdir -p %s"%d.densdir)
        f = open(os.path.join(d.densdir,'dens.dat'),'w')
        pickle.dump(d.dens_array*weight,f,protocol=pickle.HIGHEST_PROTOCOL)
        f.close()
        ...
        pickle.dump(d.dens_pure*weight,f,protocol=pickle.HIGHEST_PROTOCOL)
        f.close()
